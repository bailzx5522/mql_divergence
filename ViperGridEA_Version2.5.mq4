...
extern int       Levels = 3;
extern int       Increment = 25;  // release version use 35
...
//+
//+ OpenLossToCloseTrades - This is an early bailout strategy. If set to a
//+          negative number (e.g. -200), the algorithm will delete all 
//+          open positions if the intratrade drawdown is equal to or 
...
//+          The strategy stops opening new positions, deletes all pending
//+          orders, and begins closing all positions which are a little 
...
extern string    Note1c="--Set negative (-300) to use; Immediate exit if trade loss exceeds--";
...
extern int       MaxOrdersLiquidate = 0;
...
//+          skewed however.
//+
//+ UseVolatilityFilter - If set true, trades will only be taken if the current prices of
...
extern int       MaxSpreadToOpen = 0;

extern string    Note2d="--UseVolatilityFilter false to turn off filter--";
...
int start()
{
   ...
   else if(Mode == ACCUMULATE) // We are in the middle of a trade set
   {
      ...
      {
         if(Verbose) Print("have Closed Orders From Set - Closing all orders for a profit of $"+tradeProfit);
         endSession();
         return(0);
      }
      ...
      {
         if(Verbose) Print("Reduced Profit Target (",profitTarget,") achieved; Closing all orders");
         endSession();
         return(0);
      }
      ...
      if(buyGoalProfit < profitTarget)   // - Increment lots Buy
      {
         for(cpt=levels; cpt >= 1 && buyGoalProfit < profitTarget; cpt--)
         {
            ...
            if(Ask <= buyPrice-MarketInfo(Symbol(),MODE_STOPLEVEL)*PointValue)
            {
               ...
               else if(GetLastError()==TOO_MANY_ORDERS) Mode = LIQUIDATE;
            }
            else
            {
        ...
        ) 
      {
         ...
         if(stealthy) reduceOrdersStopLoss(STEALTH_FACTOR*increment);
      ...
      if(Seconds()==0) // log 1 min situation updates
      {
         if(Verbose && DebugPrint) 
         {
            Print("Liquidation Update: Current Open Orders: "+totalOpenOrderCount+
                  ...
                  ", buyGoalProfit=$"+DoubleToStr(buyGoalProfit,2)+
                  ", sellGoalProfit=$"+DoubleToStr(sellGoalProfit,2)+
   ...
   {
      if( AlertsEnabled ) Alert(Symbol()," ** EXCEEDED MAX INTRASET LOSS - EXITING POSITIONS! **");
      else                Print("** EXCEEDED MAX INTRASET LOSS - EXITING POSITIONS! **");
      endSession();
   }
...
isVolatile(int timeFrame)
{
   ...
   bool goodVolatility  = bandDiff / averageTrueRange > volatilityFactor;
